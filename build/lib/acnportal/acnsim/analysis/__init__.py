...
def energy_cost(sim, tariff=None):
    ...
    energy_costs = tariff.get_tariffs(sim.__start, len(agg), sim.__period)
    return np.array(energy_costs).dot(agg) * (sim.__period / 60)
...
def demand_charge(sim, tariff=None):
    ...
    if tariff is None:
        if "tariff" in sim.signals:
            ...
            raise ValueError("No pricing method is specified.")
    agg = aggregate_power(sim)
    dc = tariff.get_demand_charge(sim.__start)
...
def datetimes_array(sim):
    ...
    no_tz_start: datetime = sim.__start.replace(tzinfo=None)
    return np.array(
        [
            np.datetime64(no_tz_start + datetime.timedelta(minutes=sim.__period * i))
