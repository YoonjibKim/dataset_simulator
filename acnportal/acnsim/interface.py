...
class Interface:
    ...
    def current_datetime(self) -> datetime:
        ...
        """
        return (
                self._simulator.__start + timedelta(minutes=self.period) * self.current_time
        )
    ...
    def period(self) -> float:
        ...
        """
        return self._simulator.__period
    ...
    def get_prices(self, length: int, start: Optional[int] = None) -> np.ndarray:
        ...
        if "tariff" in self._simulator.signals:
            ...
            price_start = self._simulator.__start + timedelta(minutes=self.period) * start
            return np.array(
